...
This reduces the processing time per point from O(N^2) to O(N) by starting with a nearly sorted list.
Unfortunately, the lag time is not constant, but increases with the amplitude of the noise.
## Simulation
Here we have a moderate lag of 50ms when the the standard deviation of the noise is 1/10 the signal amplitude:
...
And with the noise almost obscuring the signal, the lag is around 200ms, which is greater than the window length.
...
Here's some data from the same source but decimated to 1 sample per minute.
